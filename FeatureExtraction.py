...
class SIFT:
    ...
    def __init__(self, gs = 8, ps = 16, gaussian_thres = 1.0, gaussian_sigma = 0.8, sift_thres = 0.2,num_angles = 12, num_bins = 5, alpha = 9.0):
        ...
        self.weights = self.get_weights(num_bins)
    ...
    def get_params_image(self, image):
        ...
        gridH = gridH.flatten()
        gridW = gridW.flatten()
        features = self.calculate_sift_grid(image, gridH, gridW)
        features = self.normalize_sift(features)
    ...
    def calculate_sift_grid(self, image, gridH, gridW):
        ...
        gaussian_height, gaussian_width = self.get_gauss_filter(self.gaussian_sigma)
        IH = self.convolution2D(image, gaussian_height)
        IW = self.convolution2D(image, gaussian_width)
    ...
    def normalize_sift(self, features):
